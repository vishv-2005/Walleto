    ...
    try {
      ...
      if (isLogin) {
        ...
        if (!savedEmail || !savedPass) {
          setError(`No account found for ${cleanedEmail}. Please signup first.`);
          setIsLogin(false);
          return;
        }

        if (cleanedEmail === savedEmail && cleanedPassword === savedPass) {
          await AsyncStorage.setItem('loggedIn', 'true');
          setEmail('');
          setPassword('');
          router.replace('/(tabs)');
        } else {
          const emailMatch = cleanedEmail === savedEmail;
          if (!emailMatch) {
            setError(`No account exists for ${cleanedEmail}.`);
          } else {
            setError('Password does not match this email.');
          }
        }
      } else {
        await AsyncStorage.setItem('userEmail', cleanedEmail);
        await AsyncStorage.setItem('userPass', cleanedPassword);
